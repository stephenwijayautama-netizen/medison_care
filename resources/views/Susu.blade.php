...
<body class="bg-gray-100 flex justify-center items-start min-h-screen py-6 font-sans">
    ...
    <div
        ...
        <header class="bg-[#009345] p-3 flex items-center gap-3 shadow-md z-30 rounded-t-3xl sticky top-0">
            ...
            <div class="flex-1">
                <input type="text" placeholder="Cari susu..."
                ...
                <div>
                    ...
                    <div class="grid grid-cols-2 gap-3">
                        @foreach($regularProducts as $item)
                            {{-- CARD PRODUK REGULAR --}}
                            <article
                                class="relative flex flex-col bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden hover:shadow-md transition duration-200 group">
                                ...
                                <div class="p-3 flex-1 flex flex-col justify-between bg-white relative">
                                    <h3
                                        class="text-[11px] font-bold text-gray-700 leading-tight mb-2 line-clamp-2 h-8 uppercase">
                                        {{ $item->product_name }}</h3> {{-- Pakai product_name sesuai DB --}}

                                    <div class="mb-3">
                                        <span class="text-[#009345] font-bold text-sm">Rp
                                            {{ number_format($item->price, 0, ',', '.') }}</span>
                                    </div>

                                    {{-- Tombol Qty --}}
                                    <div
                                        class="flex items-center justify-between bg-gray-50 rounded-lg p-1 border border-gray-100">
                                        ...
                                        <button onclick="increaseQty({{ $item->id }})"
                                            class="w-6 h-6 flex items-center justify-center bg-[#009345] text-white rounded-md shadow-sm hover:bg-green-700 transition font-bold text-md">+</button>
                                    </div>
                                </div>
                            </article>
                        @endforeach
